...
from model_utils import build_result_df, compute_rmse
...
def knn_regression(feature_train, label_train, feature_test, k=3):
    ...
    for test in feature_test:
        ...
        values = np.array([label for _, label in k_nearest])
...
def build_pattern_dataset_ohlc(returns, open_rel, high_rel, low_rel, close_rel, window=20, n_steps=1):
    ...
    for i in range(window, len(returns) - max(n_steps, 1) + 1):
        ...
        if label_idx < 0 or label_idx >= len(returns):
            continue
        label = np.array([open_rel[label_idx], high_rel[label_idx], low_rel[label_idx],  close_rel[label_idx]])
...
def run_pattern_knn(df, window, k, training_ratio, validation_ratio, testing_ratio, n_steps):
    ...
    predictions, actuals, rmse_val = walk_forward_validation(features, labels, knn_regression, data_split_ratios=(training_ratio, validation_ratio, testing_ratio), k=k)
...
def main(START_DATE="2010-01-01", END_DATE="2025-12-31",
        ...
        # Flag for evaluation during tuning
    ...
    for window in pattern_windows:
        _, predictions, actuals = run_pattern_knn(df, window=window, k=k, training_ratio=training_ratio,
            validation_ratio=validation_ratio, testing_ratio=testing_ratio, n_steps=N_STEPS,
        )

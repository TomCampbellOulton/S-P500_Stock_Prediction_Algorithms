...
def main(START_DATE="2010-01-01", END_DATE="2025-12-31",
    ...
    model     = GRUModel(feature_train.shape[2], HIDDEN_SIZE, NUM_LAYERS)
    ...
    predicted  = label_scaler.inverse_transform(predictions_scaled)   # (n_test, 4)
    actual_rel = label_scaler.inverse_transform(label_test.numpy())

    # Each sequence[split + j] has its label at df index:
    #   split + j + SEQUENCE_LENGTH + N_STEPS - 1
    # actual_opens[j] = df["open"] at that same index.
    label_start  = split + SEQUENCE_LENGTH + N_STEPS - 1
    actual_opens = df["open"].values[label_start: label_start + len(predicted)]
    dates        = df["Date"].values[label_start: label_start + len(predicted)]
    ...
    print(f"GRU ({N_STEPS}-step ahead) RMSE [{rmse_mode}]: {rmse_scores}")

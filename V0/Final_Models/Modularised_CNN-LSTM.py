...
def create_sequences(features, labels, seq_len, n_steps):
    """
    n_steps=0 → label index = i + seq_len - 1  (nowcast: last element of window)
    n_steps≥1 → label index = i + seq_len + n_steps - 1  (n steps beyond window end)
    Loop bound ensures label index never exceeds len-1.
    """
    X, y = [], []

...
from upgraded_utilities import feature_engineering, get_data, get_features_and_labels, get_feature_names, dtr_walk_forward_validation
...
def main(START_DATE="2010-01-01", END_DATE="2025-12-31",
    ...
    # Capture feature names BEFORE converting to numpy (get_features_and_labels returns arrays)
    ...
    predicted, actual, _, test_indices = dtr_walk_forward_validation(
        ...
        data_split_ratios=DATA_SPLIT_RATIOS, max_depth=1, min_samples_split=5,
        min_samples_leaf=15, k=None,
    )

    actual_opens = df["open"].values[test_indices]   # ← exact rows, not a fixed slice
    dates        = df.index[test_indices]            # ← same
    seed_open    = df["open"].values[test_indices[0] - 1]   # last known training open

# model_utils.py — shared helpers used by all models
#
# build_targets(df, n_steps)
#   Creates target_open/high/low/close as ratios relative to current open.
#   target_X[t] = X[t + n_steps] / open[t]
#
# compute_rmse(predicted_rel, actual_rel, actual_opens, mode)
#   mode="price"    → converts relatives → prices per row using actual_opens, then RMSE
#   mode="relative" → RMSE directly on the relative values
#
# build_result_df(predicted_rel, actual_opens)
#   Returns a DataFrame with 8 columns:
#   open_rel, high_rel, low_rel, close_rel, open, high, low, close
#
#   IMPORTANT: actual_opens must be the open price at each prediction's reference time t
#   (the denominator used in build_targets), NOT a single seed value.
...
def build_result_df_chained(predicted_rel: np.ndarray,
    ...
    """
    Reconstruct absolute prices from a single seed open — no test-set data used.

    WHY SUBSAMPLING:
    Each predicted_rel[t] = value[t + n_steps] / open[t].  Chaining every step
    compounds an n-step-ahead ratio 750 times, giving (price_ratio)^n_steps drift
    (~10x for n_steps=5 on a rising market).  The fix is to chain only every
    n_steps-th prediction so consecutive links are non-overlapping — their
    numerators and denominators telescope correctly — then linearly interpolate
    the intermediate steps for display.

    For HIGH / LOW / CLOSE at each step t we preserve the model's intraday
    spread by applying (predicted_high[t] / predicted_open[t]) to the chained open.

    Parameters
    ----------
    predicted_rel : (n, 4)  [open_rel, high_rel, low_rel, close_rel]
    seed_open     : float   last known open before the test window (training data only)
    n_steps       : int     prediction horizon used when building targets
    idx           : timestamps or None
